...
const projects = [
  {
    ...
    isLive: true,
    cursorLabel: 'A Web Application That Automates Workflows for Small Businesses ↗',
  },
  {
    id: '02',
    title: 'StorageVault',
    ...
    isLive: true,
    cursorLabel: 'A Secure Unlimited Cloud Storage Solution Using MinIO & Next.js ↗',
  },
  {
    id: '03',
    ...
    image: '',
    isLive: false,
    cursorLabel: 'Real-Time DDoS Detection Using Machine Learning Wich can detect DDoS attacks and Inform The User ↗',
  },
];
  ...
  return (
    <section className="py-16 md:py-32 px-4 sm:px-6 md:px-12 bg-surface-container-low" id="work">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Cursor Follow Area for Featured Projects */}
        <CursorFollow defaultText="VIEW PROJECT ↗" className="space-y-4">
          {projects.map((project, index) => (
            <a
              ...
              data-cursor-text={project.cursorLabel}
              className={`group block relative py-6 sm:py-8 md:py-16 border-t ${
                index === projects.length - 1 ? 'border-b' : ''
              } border-outline-variant/10 overflow-hidden transition-colors duration-500 hover:bg-surface-container-highest/20 cursor-pointer no-underline text-inherit rounded-4xl hover:shadow-[0_10px_10px_rgba(200,255,0,0.09)]`}
            >
              {project.isLive && (
                <div className="absolute top-4 right-4 md:top-6 md:left-8 md:right-auto z-20">
                  <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-black/40 backdrop-blur-md border border-primary-fixed/30 shadow-[0_0_12px_rgba(200,255,0,0.2)]">
              ...
              <div className="flex flex-col md:flex-row md:items-end justify-between relative z-10 transition-all duration-500 group-hover:px-2 md:group-hover:px-8 gap-3 md:gap-4">
                <div className="flex items-baseline gap-4 md:gap-8">
                  <span className="font-headline font-bold text-primary-fixed text-base md:text-lg transition-transform duration-500 group-hover:-translate-y-2">
                    {project.id}
                  </span>
              ...
              {project.image && (
                <div
                  className={`hidden md:block absolute ${
                    index % 2 === 0 ? 'right-[10%]' : 'right-[15%]'
                  } top-1/2 -translate-y-1/2 w-[350px] md:w-[400px] aspect-video opacity-0 group-hover:opacity-100 transition-all duration-700 pointer-events-none scale-75 group-hover:scale-110 z-0 group-hover:shadow-[0_0_50px_rgba(200,255,0,0.15)] rounded-xl overflow-hidden border border-white/5 bg-surface-container-highest`}
                >
                  <Image
                    src={project.image}
